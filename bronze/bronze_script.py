...
# --------------------------------------------------------------------
sequencing_run_src_df = (
    spark.read.format(FILE_FORMAT)
    .option("header", "true")
    .load(f"{SOURCE_PATH}/genomics_sequencing_runs_5000.{FILE_FORMAT}/")
)
sequencing_run_src_df.createOrReplaceTempView("genomics_sequencing_runs_5000")
...
lab_result_src_df.createOrReplaceTempView("lab_test_results_5000")
...
# --------------------------------------------------------------------
# genomic_variant_bronze
# --------------------------------------------------------------------
genomic_variant_src_df = (
    spark.read.format(FILE_FORMAT)
    .option("header", "true")
    .load(f"{SOURCE_PATH}/genomic_variants_5000.{FILE_FORMAT}/")
)
genomic_variant_src_df.createOrReplaceTempView("genomic_variants_5000")

genomic_variant_bronze_df = spark.sql(
    """
    SELECT
...
(
    genomic_variant_bronze_df.coalesce(1)
    .write.mode("overwrite")
    .format("csv")
    .option("header", "true")
    .save(f"{TARGET_PATH}/genomic_variant_bronze.csv")
)
